/**
 * 面板图管理 — 上传、删除、查看面板图列表
 * 命令: #上传xxx面板图 / #删除xxx面板图N / #xxx面板图列表
 */
import { createEvent, EventsEnum, Format, useMessage } from 'alemonjs';
import { queryMihoyoApi } from 'alemonjs-mhy';

type ImgAction = 'upload' | 'delete' | 'list';

function parseImgAction(text: string): { action: ImgAction; charName: string; index?: number } {
...
export default async (e: EventsEnum) => {
  ...
  switch (action) {
    case 'upload': {
      ...
      md.addText(result.success ? (result.data as { message: string }).message : `[上传面板图] ${result.message}`);
      format.addMarkdown(md);
      void message.send({ format });
    ...
    case 'delete': {
      ...
      md.addText(result.success ? (result.data as { message: string }).message : `[删除面板图] ${result.message}`);
      format.addMarkdown(md);
      void message.send({ format });
    ...
    case 'list': {
      ...
      if (result.success) {
        const data = result.data as { images?: string[]; message?: string };
